...
import MDBadge from "ResuseableComponents/MDBadge";

// Material Dashboard 2 PRO React components
import Breadcrumbs from "components/Breadcrumbs";
import NotificationItem from "components/Items/NotificationItem";
import MyForm from "components/sharedashboard"// import DefaultItem from "components/Items/DefaultItem";
...
function DashboardNavbar({ absolute, light, isMini }) {
  ...
  const handleSelectYearClick=(year)=>{
    ceo.actions.setSelectedYear(year)
  }
    ...
    >
      <Toolbar sx={(theme) => navbarContainer(theme)}>
        <MDBox color="inherit" mb={{ xs: 1, md: 0 }} sx={(theme) => navbarRow(theme, { isMini })}>
          ...
          {(ceo.states.transformedData && route.includes('analytics')) &&
                        <ButtonGroup
                          size="small"
                        ...
                        .map((data) => {
                          const key = data[0];
                          return (
                                <Button variant="contained" color="black" disabled={key===ceo.states.selectedYear} onClick={()=>handleSelectYearClick(key)}>
                                  {key}
                                </Button>
                                )
                          })
        ...
        {isMini ? null : (
          <MDBox sx={(theme) => navbarRow(theme, { isMini })}>
            <MDBox pr={1}>
              <MDInput label="Search here haris" />
            </MDBox>
