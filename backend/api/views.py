...
from .serializers import (
    DashboardSerializer, DataSourceSerializer, ShareSerializer, ChangeRoleSerializer, GoogleSheetSerializer
)
from .models import Dashboard, Share, DataSource
...
class ChartsView(APIView):
    ...
    def get(self, request, dash_id):
        ...
        firestore = Firestore(project_id=os.getenv("PROJECT_ID"), collection="charts")
        charts = firestore.read_collection(_id=f"{company_slug}-{dashboard.id}")
        if charts:
            return Response(charts[0], status=status.HTTP_200_OK)
